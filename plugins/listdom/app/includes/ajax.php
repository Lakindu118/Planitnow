...
class LSD_Ajax extends LSD_Base
{
    ...
    {
        // Get Map Objects
        add_action('wp_ajax_lsd_ajax_search', array($this, 'search'));
        add_action('wp_ajax_nopriv_lsd_ajax_search', array($this, 'search'));
        ...
        add_action('wp_ajax_lsd_hierarchical_terms', array($this, 'terms'));
        add_action('wp_ajax_nopriv_lsd_hierarchical_terms', array($this, 'terms'));
    }
    ...
    public function terms()
    {
        ...
        // Verify that the nonce is valid
        if(!wp_verify_nonce(sanitize_text_field($_REQUEST['_wpnonce']), 'lsd_search_form')) $this->response(array('success' => 0, 'message' => esc_html__("Security nonce is invalid.", 'listdom')));

        // Taxonomy
        $taxonomy = (isset($_REQUEST['taxonomy']) ? $_REQUEST['taxonomy'] : LSD_Base::TAX_LOCATION);
