...
class LSD_Skins extends LSD_Base
{
    ...
    public function start($atts)
    {
        ...
        // Skin Options
        $this->skin_options = (isset($this->atts['lsd_display']) and isset($this->atts['lsd_display'][$this->skin])) ? $this->atts['lsd_display'][$this->skin] : [];
        ...
        $this->display_image = ($this->isLite() or !isset($this->skin_options['display_image']) or $this->skin_options['display_image']);
        $this->image_method = (isset($this->skin_options['image_method']) and $this->skin_options['image_method']) ? $this->skin_options['image_method'] : 'cover';
        $this->load_more = (isset($this->skin_options['load_more']) and $this->skin_options['load_more']);
        $this->pagination = $this->skin_options['pagination'] ?? (!$this->load_more ? 'disabled' : 'loadmore');
        $this->display_labels = (isset($this->skin_options['display_labels']) and $this->skin_options['display_labels']);
        $this->display_share_buttons = (isset($this->skin_options['display_share_buttons']) and $this->skin_options['display_share_buttons']);
        ...
        // Map Search Options
        $this->mapsearch = (isset($this->skin_options['mapsearch']) and $this->skin_options['mapsearch']);
        $this->autoGPS = (isset($this->skin_options['auto_gps']) and $this->skin_options['auto_gps']);
        $this->maxBounds = apply_filters('lsd_map_max_bounds', ((isset($this->skin_options['max_bounds']) and is_array($this->skin_options['max_bounds'])) ? $this->skin_options['max_bounds'] : []));

        // HTML Class
        $this->html_class = (isset($this->atts['html_class']) and trim($this->atts['html_class'])) ? sanitize_text_field($this->atts['html_class']) : '';

        // Is it Widget?
        $this->widget = (isset($this->atts['widget']) and $this->atts['widget']);
    ...
    public function filter()
    {
        ...
        $response = ['success'=>1, 'html'=>LSD_Kses::page($output), 'next_page'=>$this->next_page, 'count'=>count($this->listings), 'total'=>$this->found_listings, 'seed'=>$this->atts['seed'] ?? null];
    ...
    public function get_search_module()
    {
        ...
        return do_shortcode('[listdom-search id="'.$this->sm_shortcode.'" page="'.(is_singular() && $post && isset($post->ID) ? $post->ID : '').'" shortcode="'.$shortcode_id.'" ajax="'.$this->sm_ajax.'"]');
    }
    ...
    {
        if($this->found_listings <= $this->limit and !$this->sm_ajax) return '';
    ...
    public function get_title_tag($listing)
    {
        ...
        // Link is Enabled
        if(in_array($listing_link_method, ['normal', 'blank']))
        {
            return '<a
                data-listing-id="'.esc_attr($listing->id()).'"
                href="'.esc_url(get_the_permalink($listing->post->ID)).'"
                '.($listing_link_method === 'blank' ? 'target="_blank"' : '').'
                '.lsd_schema()->url().'
            >'.LSD_Kses::element($listing->get_title()).'</a>';
        }
        // Link is Disabled
