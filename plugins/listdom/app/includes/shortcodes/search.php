...
class LSD_Shortcodes_Search extends LSD_Shortcodes
{
    ...
    public function row($row)
    {
        ...
        $filters = (isset($row['filters']) and is_array($row['filters']) and count($row['filters'])) ? $row['filters'] : [];
    ...
    public function field_taxonomy_hierarchy($name, $current, $all_terms, $predefined_terms, $terms, $render_type)
    {
        ...
        foreach($terms as $key => $term)
        {
            ...
            if($render_type === 'checkboxes') $render .= '<label><input type="checkbox" class="'.esc_attr($key).'" name="'.esc_attr($name).'[]" value="'.esc_attr($key).'" '.(in_array($key, $current) ? 'checked="checked"' : '').'>'.esc_html($term["name"]).'</label>';
    ...
    public function field_period($filter)
    {
        ...
        $output = '<div class="lsd-search-filter '.esc_attr($this->col_filter).'">';
        $output .= '<label for="'.esc_attr($id).'">'.esc_html__($title, 'listdom').'</label>';
        $output .= '<input type="text" class="lsd-date-range-picker" data-format="'.strtoupper(esc_attr($format)).'" data-periods="'.htmlspecialchars(json_encode($months), ENT_QUOTES, 'UTF-8').'" name="'.esc_attr($name).'" id="'.esc_attr($id).'" placeholder="'.esc_attr__($placeholder, 'listdom').'" value="'.esc_attr($current).'">';
        $output .= '</div>';
