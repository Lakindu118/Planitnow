...
class LSD_Notifications_Email_Prepare extends LSD_Notifications
{
    ...
    public function new_listing($listing_id)
    {
        ...
        foreach($notifications as $notification)
        {
            ...
            // Trigger Action
            do_action('lsd_send_notification', $this->admin_id(), array(
    ...
    public function listing_status_changed($listing_id, $previous)
    {
        ...
        foreach($notifications as $notification)
        {
            ...
            // Trigger Action
            do_action('lsd_send_notification', $owner_id, array(
                'previous_status' => (isset($status->label) ? $status->label : ''),
                'owner_name' => $owner_name,
                'owner_email' => $owner_email,
            ), $notification->ID, $listing_id);
        }
    ...
    public function form($args, $hook)
    {
        ...
        foreach($notifications as $notification)
        {
            ...
            // Trigger Action
            do_action('lsd_send_notification', ($hook === 'lsd_listing_report_abuse' ? $this->admin_id() : $owner_id), array(
                'name' => $name,
                'email' => $email,
                'phone' => $phone,
                'message' => $message,
